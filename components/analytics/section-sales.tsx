    ...
    <SectionWrapper title="Sales">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
        ...
        <KpiCard
          ...
          value={fmt(kpis.total_discounts)}
          accent="warning"
        />
      </div>
            ...
            <BarChart data={peak_hours}>
              ...
              <Bar dataKey="avg_orders" radius={[4, 4, 0, 0]}>
                {peak_hours.map((h, i) => {
                  const isHighest =
                    h.avg_orders ===
                    Math.max(...peak_hours.map((x) => x.avg_orders));
                  return (
                    <Cell key={i} fill={isHighest ? "#f59e0b" : "#3b82f6"} />
                  );
                })}
              </Bar>
            </BarChart>
