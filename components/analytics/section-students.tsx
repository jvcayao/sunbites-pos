...
export function SectionStudents({ data }: Props) {
  ...
  const typeDist = [
    ...
    { name: "Non-Subscription", value: kpis.non_subscription_count, fill: "#10b981" },
  ];
  ...
  return (
    <SectionWrapper title="Students">
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-7">
        ...
        <KpiCard label="New (period)"   value={kpis.new_enrollments} accent="success" />
        <KpiCard label="Upgrades"       value={kpis.subscription_upgrades}   accent="success" />
          ...
          {switch_trend.every((p) => p.upgrades === 0 && p.downgrades === 0) ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={switch_trend}>
                ...
                <Bar dataKey="upgrades"   fill="#10b981" radius={[4, 4, 0, 0]} name="Upgrades"   />
                <Bar dataKey="downgrades" fill="#ef4444" radius={[0, 0, 4, 4]} name="Downgrades" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
