  ...
  return (
    <SectionWrapper title="Billing">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
        ...
        <KpiCard label="Collection Rate" value={`${kpis.collection_rate}%`}  accent={rateAccent} />
        <KpiCard label="Discrepancies"   value={kpis.discrepancy_count}      accent={kpis.discrepancy_count > 0 ? "warning" : "default"} />
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={monthly_trend}>
              ...
              <Tooltip />
              <Bar dataKey="paid_count"   stackId="a" fill="#10b981" name="Paid"    radius={[0, 0, 0, 0]} />
              <Bar dataKey="unpaid_count" stackId="a" fill="#ef4444" name="Unpaid"  radius={[0, 0, 0, 0]} />
              <Bar dataKey="void_count"   stackId="a" fill="#6b7280" name="Voided"  radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={monthly_trend}>
              ...
              <ReferenceLine y={95} stroke="#10b981" strokeDasharray="4 2" label={{ value: "95%", position: "right", fontSize: 10 }} />
              <Area
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left">
                    ...
                    <th className="pb-2 pr-4 text-right font-medium text-green-600">Paid</th>
                    <th className="pb-2 pr-4 text-right font-medium text-red-500">Unpaid</th>
                    <th className="pb-2 text-right font-medium text-gray-400">Voided</th>
                  </tr>
                </thead>
                <tbody className="divide-y">
                  {by_grade.map((r) => (
                    <tr key={r.grade_level} className="tabular-nums">
                      ...
                      <td className="py-1.5 pr-4 text-right text-green-600">{r.paid}</td>
                      <td className="py-1.5 pr-4 text-right text-red-500">{r.unpaid}</td>
                      <td className="py-1.5 text-right text-gray-400">{r.void}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
