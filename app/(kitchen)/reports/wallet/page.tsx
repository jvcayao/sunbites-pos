    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex items-center gap-2">
          <label htmlFor="wallet-from" className="text-xs text-muted-foreground">From</label>
          <input
            id="wallet-from"
            type="date"
            value={dateFrom}
        ...
        <div className="flex items-center gap-2">
          <label htmlFor="wallet-to" className="text-xs text-muted-foreground">To</label>
          <input
            id="wallet-to"
            type="date"
            value={dateTo}
      ...
      {summary && summary.students_below_100 > 0 && (
        <div className="flex items-center gap-2 rounded-lg border border-yellow-300 bg-yellow-50 p-3">
          ...
          <p className="text-sm text-amber-800">
            <span className="font-semibold">{summary.students_below_100} student{summary.students_below_100 !== 1 ? "s" : ""}</span> have a wallet balance below ₱100.00. Consider sending a top-up reminder.
          </p>
        </div>
      )}
      ...
      {summary && (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <SummaryCard label="Total Credits" value={formatPeso(summary.total_credits)} colorClass="border-green-200 bg-green-50" />
          <SummaryCard label="Total Debits" value={formatPeso(summary.total_debits)} colorClass="border-red-200 bg-red-50" />
          <SummaryCard label="Net Movement" value={formatPeso(summary.net_movement)} />
          <SummaryCard label="Students Below ₱100" value={String(summary.students_below_100)} colorClass="border-yellow-200 bg-yellow-50" />
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40">
              <tr>
                ...
                <th className="px-4 py-2 text-right text-xs font-semibold text-muted-foreground">Total Credited</th>
                <th className="px-4 py-2 text-right text-xs font-semibold text-muted-foreground">Total Debited</th>
                <th className="px-4 py-2 text-left text-xs font-semibold text-muted-foreground">Last Transaction</th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                ...
                rows.map((row) => (
                  <tr key={row.id} className="hover:bg-muted/20">
                    ...
                    <td className={cn("px-4 py-2.5 text-right font-semibold", row.current_balance < 100 && "text-amber-600")}>
                      {formatPeso(row.current_balance)}
                    </td>
                    <td className="px-4 py-2.5 text-right text-red-600">{row.outstanding_credit > 0 ? formatPeso(row.outstanding_credit) : "—"}</td>
                    <td className="px-4 py-2.5 text-right text-green-700">{formatPeso(row.total_credited)}</td>
                    <td className="px-4 py-2.5 text-right text-muted-foreground">{formatPeso(row.total_debited)}</td>
                    <td className="px-4 py-2.5 text-muted-foreground">{formatDate(row.last_transaction)}</td>
                  </tr>
                ))
              )}
            </tbody>
